...
class _InvoiceTableState extends State<InvoiceTable> {
  ...
  void deleteRow() {
    ...
    if (tableValues.isNotEmpty) {
      ...
      tableValuesProvider
          .changeTableValues(tableValues); // Update provider directly
    }
  }
    ...
    for (var item in tableValues) {
      ...
      if (qty == 0 || rate == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Invalid Quantity/Rate in some items')),
        );
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.initialTableValues != null &&
          widget.initialTableValues!.isNotEmpty) {
        tableValues =
            List<Map<String, dynamic>>.from(widget.initialTableValues!);
      } else {
        tableValues = [
          {
            'image': 'assets/images/noImage.png',
                    ...
                    children: [
                      ...
                      TableCell(
                        child: Padding(
                          padding: const EdgeInsets.only(left: 8.0, right: 8),
                          child: TextFormField(
                            initialValue: tableValues[index]['size'],
                            onChanged: (value) {
                              if (index < tableValues.length) {
                                tableValues[index]['size'] = value;
                              } else {
                                tableValues.add({'size': value});
                              }
                              context
                                  .read<TableValuesProvider>()
                                  .changeTableValues(tableValues);
                            },
                          ),
                        ),
                      ),
                      TableCell(
                        child: Padding(
                          ...
                          child: TextFormField(
                            ...
                            initialValue:
                                tableValues[index]['rate']?.toString() ?? '0',
                            onChanged: (value) {
                              if (index < tableValues.length) {
                                tableValues[index]['rate'] = value;
                                double rate = double.tryParse(value) ?? 0;
                                double qty = double.tryParse(
                                        tableValues[index]['qty']) ??
                                    1.0;
                                amount = qty * rate;
                                tableValues[index]['amount'] =
                                    amount.toStringAsFixed(2);
                              } else {
                                tableValues
                                    .add({'rate': value, 'amount': '0.00'});
                              }
                              context
                                  .read<TableValuesProvider>()
                                  .changeTableValues(tableValues);
